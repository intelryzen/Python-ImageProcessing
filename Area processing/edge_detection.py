...
def apply_prewitt(image):
    ...
    # 쓰레시홀딩
    thres = np.where(output >= 50, 255, 0).astype(np.uint8)
    show_img(thres, title=f"prewitt")


def apply_prewitt_color(image):
    ...
    kernel_height = kernel_x.shape[0]
    padded_image = apply_zero_padding(intensity, kernel_x)
...
if __name__ == "__main__":
    ...
    # apply_sobel(img)
    # apply_prewitt(img)
    apply_prewitt_color(img2)
    apply_sobel_color(img2)
